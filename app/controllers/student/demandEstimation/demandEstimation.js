...
module.exports = {
  ...
  getCoursesByAreaName :(req , res) => {
    Promise.all([demandEstimation.getCourseListByAreaName(res.locals.slug, res.locals.biddingId, 
                 req.body.  showEntry, req.body.acadSessionId, req.body.areaName),demandEstimation.getCountOfCourseListByAreaName(res.locals.slug, res.locals.biddingId, req.body.acadSessionId, req.body.areaName)]).then(result => {
                 res.json({
                  status: "200",
                  message: "Sucessfull",
                  courseList: result[0].recordset,
