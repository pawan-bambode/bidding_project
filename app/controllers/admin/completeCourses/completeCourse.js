...
module.exports = {
    ...
    getCompleteCourses: (req, res) => {
        Promise.all([
            completeCourses.getCompleteCourseList(res.locals.slug, res.locals.biddingId, req.body.showEntry),
            completeCourses.getCount(res.locals.slug, res.locals.biddingId)
        ]).then(result => {
    ...
    generateExcel: (req, res) => {
        ...
        const filePath = path.join(__dirname, '/sampleForImportCompleteCourses.xlsx');
    ...
    readExcelFile: (req, res) => {
        ...
        let completeCourseDataValue = {
            completed_courses: completeCoursesWithColumnHypen
        };

        completeCourses.uploadCompleteCoursesData(res.locals.slug, completeCourseDataValue, res.locals.userId, biddingId)
            .then(result => {
    ...
    deleteAll: (req, res) => {
        completeCourses.deleteAll(res.locals.slug, res.locals.biddingId, res.locals.userId, req.body)
            .then(result => {
                res.status(200).json(JSON.parse(result.output.output_json));
            })
            .catch(error => {
    ...
    showEntry: (req, res) => {
        Promise.all([
            completeCourses.showEntry(res.locals.slug, res.locals.biddingId, req.body.showEntry, req.body.pageNo),
            completeCourses.getCounts(res.locals.slug, res.locals.biddingId)
        ]).then(result => {
            res.json({
                status: '200',
    ...
    search: (req, res) => {
        Promise.all([
            completeCourses.search(res.locals.slug, res.locals.biddingId, req.body.searchLetter, req.body.pageNo, req.body.showEntry),
            completeCourses.getCountSearch(res.locals.slug, res.locals.biddingId, req.body.searchLetter, req.body.pageNo, req.body.showEntry)
        ]).then(result => {
            res.json({
                status: "200",
                message: "Result fetched",
