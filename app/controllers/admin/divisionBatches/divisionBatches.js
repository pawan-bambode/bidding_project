...
module.exports = {
    ...
    delete :(req,res) =>{
     divisionBatch.delete(req.body.id,res.locals.slug,res.locals.biddingId,res.locals.userId).then(result =>{    
        res.status(200).json(JSON.parse(result.output.output_json));
     }).catch(error =>{
        if(isJsonString.isJsonString(error.originalError.info.message)){
            res.status(500).json(JSON.parse(error.originalError.info.message));
    ...
    deleteAll:(req,res) =>{
        divisionBatch.deleteAll(res.locals.slug,res.locals.biddingId,res.locals.userId,req.body).then(result =>{
               res.status(200).json(JSON.parse(result.output.output_json));
           })
           .catch(error => {
               if (isJsonString.isJsonString(error.originalError.info.message)) {
    ...
    search :(req,res) =>{  
     Promise.all([divisionBatch.search(res.locals.slug,res.locals.biddingId,req.body.searchLetter,res.locals.userId,req.body.showEntry,req.body.pageNo),divisionBatch.getCountSearch(res.locals.slug,res.locals.biddingId,req.body.searchLetter,res.locals.userId)]).then(result => {
            res.json({
                status: "200",
...
showEntryDivisionBatchesList :(req,res) =>{
  Promise.all([divisionBatch.showEntryDivisionBatchesList(res.locals.slug,res.locals.biddingId,req.body.showEntry,req.body.pageNo),divisionBatch.getCounts(res.locals.slug,res.locals.biddingId)]).then(result =>{
        res.json({
            status:'200',
            message:'Result fetched',
            data:result[0].recordset,
