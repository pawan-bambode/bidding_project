...
module.exports = {
  getPreRequities: (req, res) => {
    Promise.all([
      preRequisites.getPreRequities(res.locals.slug, res.locals.biddingId, req.body.showEntry),
  ...
  readExcelFile: (req, res) => {
    ...
    preRequisites.uploadPreRequisites(res.locals.slug, preRequisitesDataValue, res.locals.userId, biddingId)
      .then(result => {
  ...
  showEntryList: (req, res) => {
    Promise.all([
      preRequisites.showEntryList(res.locals.slug, res.locals.biddingId, req.body.showEntry, req.body.pageNo),
      preRequisites.getCounts(res.locals.slug, res.locals.biddingId)
    ]).then(result => {
      res.json({
        status: '200',
  ...
  search: (req, res) => {
    Promise.all([
      ...
      preRequisites.getCountSearch(res.locals.slug, res.locals.biddingId, req.body.searchLetter)
    ]).then(result => {
      res.json({
        status: "200",
        message: "Result fetched",
