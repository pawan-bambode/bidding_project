...
module.exports = class DemandEstimation
{
    ...
    static getCourseListByAcadSession(slug,biddingId,showEntry,acadSessionId){
        return poolConnection.then(pool =>{
            ...
            .query(`SELECT TOP  ${showEntry} c.*,p.program_name FROM [${slug}].courses c INNER JOIN [${slug}].programs p  ON c.program_id = p.program_id WHERE
    ...
    static getCourseCountByAcadSession(slug,biddingId,acadSessionId){
        return poolConnection.then(pool =>{
            ...
            .input('biddingId',sql.Int,biddingId)
            .input('acadSessionId',sql.Int,acadSessionId)
            .query(`SELECT COUNT(*) AS count FROM [${slug}].courses c INNER JOIN [${slug}].programs p  ON c.program_id = p.program_id WHERE
            c.bidding_session_lid = @biddingId AND c.sap_acad_session_id = @acadSessionId AND c.active = 1`)     
        })
    }
    ...
    static getAreaList(slug,biddingId,acadSessionId){
        return poolConnection.then(pool =>{
            ...
            .input('biddingId',sql.Int,biddingId)
            .input('acadSessionId',sql.Int,acadSessionId)    
            .query(`SELECT DISTINCT area_name FROM [${slug}].courses WHERE bidding_session_lid = @biddingId  AND sap_acad_session_id = @acadSessionId  AND active = 1`);
        })
    }
    ...
    static getCourseListByAreaName(slug,biddingId,showEntry,acadSessionId,areaName){
        return poolConnection.then(pool =>{
            ...
            .query(`SELECT TOP ${showEntry} c.*,p.program_name FROM [${slug}].courses c INNER JOIN [${slug}].programs p  ON c.program_id = p.program_id WHERE c.sap_acad_session_id = @acadSessionId AND c.active = 1 AND c.bidding_session_lid = @biddingId AND c.area_name LIKE @areaName`)     
        })
    }
    ...
    static getCountOfCourseListByAreaName(slug,biddingId,acadSessionId,areaName){
        return poolConnection.then(pool =>{
            ...
            .input('acadSessionId',sql.Int,acadSessionId)
            .input('areaName', sql.NVarChar, `%${areaName}%`)
            .query(`SELECT COUNT(*) AS count FROM [${slug}].courses c INNER JOIN [${slug}].programs p  ON c.program_id = p.program_id WHERE c.sap_acad_session_id = @acadSessionId AND c.active = 1 AND c.bidding_session_lid = @biddingId AND c.area_name LIKE @areaName`)     
        })
    }
                ...
                return poolConnection.then(pool => {
                    return pool.request()
                        ...
                        .query(`SELECT c.id, course_name, credits, program_id, ad.acad_session, area_name, min_demand_criteria, year_of_introduction
                            ...
                            WHERE c.bidding_session_lid = @bidding_session_lid AND active = '1' AND c.sap_acad_session_id = @acadSessionId AND c.area_name LIKE @area AND (course_name LIKE @letterSearch OR credits LIKE  @letterSearch OR program_id LIKE @letterSearch OR ad.acad_session LIKE @letterSearch OR area_name LIKE @letterSearch  OR year_of_introduction LIKE @letterSearch OR min_demand_criteria LIKE @letterSearch) ORDER BY c.id DESC  OFFSET (@pageNo - 1) * ${showEntry} ROWS FETCH NEXT ${showEntry} ROWS ONLY
                        `);
                })
            }
    ...
    static getSelectedCourses(slug,biddingId,studentId){
        console.log('values of student',studentId);
        return poolConnection.then(pool =>{
            return pool.request()
            .input('biddingId',sql.Int,biddingId)
