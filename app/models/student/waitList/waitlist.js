  ...
  static getStudentDetails(slug, biddingId, studentId) {
    return poolConnection.then((pool) => {
      return pool
        ...
        .query(`SELECT concentration_lid AS concentrationId FROM [${slug}].student_data where id = @studentLid AND bidding_session_lid = @biddingId`);
    });
  }
    ...
    let roundId = 6;
          return poolConnection.then(pool =>{
              ...
              .query(`SELECT t.division_batch_lid, 
                      ...
                      d.day_name, c.id AS course_lid, IIF(sem.is_favourite IS NULL,0, sem.is_favourite) AS is_favourite, seb.bid_points, seb.id
