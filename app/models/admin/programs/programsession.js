...
module.exports = class ProgramSession{
    ...
    static getCredits(slug,biddingId){
        return poolConnection.then(pool =>{
            return pool.request()
            .input('biddingId',sql.Int,biddingId)
            .query(`SELECT ad.sap_acad_session_id,ad.acad_session ,ps.min_credits ,ps.max_credits FROM [${slug}].program_sessions ps 
    ...
    static getProgramSessionCreditsPoint(slug,biddingId){
        return poolConnection.then(pool =>{
            return pool.request()
            .input('biddingId',sql.Int,biddingId)
            .query(`SELECT SUM(IIF(max_credits IS NULL, 0, max_credits)) AS max_yearly_credits , SUM(IIF(min_credits IS NULL, 0, min_credits)) AS min_yearly_credits FROM [${slug}].program_sessions
            WHERE bidding_session_lid = @biddingId AND active = 1`)
        })
    }
    ...
    static search(pageNo,letterSearch,showEntry,slug,biddingId,userId){
        ...
        if(pageNo){
         return poolConnection.then(pool =>{
             ...
             .query(`SELECT  ps.id ,p.program_name,ad.acad_session,bs.year, IIF(ps.min_credits IS NULL, 0, ps.min_credits) AS min_credits,IIF(ps.max_credits IS NULL, 0, ps.max_credits) AS max_credits
             ...
             ORDER BY ps.id   OFFSET (@pageNo - 1) * ${showEntry} ROWS FETCH NEXT ${showEntry} ROWS ONLY`)
         })
     } else{
         return poolConnection.then(pool =>{
             return pool.request()
    ...
    static showEntryProgramSessionList(showEntry,slug,biddingId,pageNo){
        ...
        if(pageNo){
            return poolConnection.then(pool=>{
              ...
              ORDER BY c.id   OFFSET (@pageNo - 1) * ${showEntry} ROWS FETCH NEXT ${showEntry} ROWS ONLY`)
            })
        }
        else{
            return poolConnection.then(pool=>{
    ...
    static getCountsProgramSession(showEntry,slug,biddingId){
        return poolConnection.then(pool =>{
            return pool.request()
            .input('biddingId',sql.Int,biddingId)
            .query(`SELECT  COUNT(*) FROM [${slug}].program_sessions ps 
                INNER JOIN [${slug}].programs p ON ps.program_id = p.program_id AND p.bidding_session_lid = @biddingId
                INNER JOIN [${slug}].bidding_session bs ON ps.bidding_session_lid = bs.id
                INNER JOIN [dbo].acad_sessions ad ON ps.sap_acad_session_id = ad.sap_acad_session_id  AND ps.bidding_session_lid = @biddingId`);
        })
