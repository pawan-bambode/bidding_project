      ...
      static getAllBiddingSession(slug,status) {
       if(status)
        return poolConnection.then(pool => {
            return pool.request()
        .query(`SELECT bs.id , RTRIM(LTRIM(bs.bidding_name)) AS biddingName,STRING_AGG(CONCAT(bas.acad_session, ':', bas.acad_session_lid), ',') AS acad_sessions_and_ids,
        bs.start_date, bs.end_date ,sd.student_count, c.course_count ,ps.max_credits ,ps.min_credits FROM [${slug}].bidding_session bs
        INNER JOIN [${slug}].bidding_acad_sessions bas ON bas.bidding_session_lid = bs.id 
        LEFT JOIN ( SELECT count(*) as student_count, sd.bidding_session_lid FROM [${slug}].student_data sd WHERE sd.active = 1
        GROUP BY bidding_session_lid ) sd ON sd.bidding_session_lid = bs.id
        LEFT JOIN ( SELECT count(*) as course_count, c.bidding_session_lid FROM [${slug}].courses c WHERE c.active = 1
