    ...
    static search(slug, biddingId, letterSearch, pageNo, showEntry) {
        ...
        return poolConnection.then(pool => {
            return pool.request()
                ...
                .input('biddingId', sql.Int, biddingId)
                .input('letterSearch', sql.NVarChar, `%${letterSearch}%`)
                .query(`SELECT pr.id, p.program_name pr.acad_session, pr.id, pr.course_name, pr.course_id,
                        type, pre_req_course_name, pre_req_course_id 
                        FROM [${slug}].pre_requisites pr
                        INNER JOIN [${slug}].courses c ON pr.course_id = c.course_id AND c.bidding_session_lid = @biddingId
            ...
            return poolConnection.then(pool => {
                return pool.request()
                    .input('biddingId', sql.Int, biddingId)
                    .input('letterSearch', sql.NVarChar, `%${letterSearch}%`)
                    .query(`SELECT pr.id, p.program_name, pr.acad_session, pr.id, pr.course_name,
    ...
    static showEntryList(slug, biddingId, showEntry, pageNo){
        ...
        if(pageNo){
            ...
            .query(`SELECT pr.id, p.program_name, pr.acad_session, pr.id, pr.course_name, pr.course_id,
                    ...
                    INNER JOIN [${slug}].courses c ON pr.course_id = c.course_id AND c.bidding_session_lid = @biddingId
                    INNER JOIN [${slug}].programs p ON p.program_id = c.program_id AND p.bidding_session_lid = @biddingId
                    WHERE pr.active = 1 AND pr.bidding_session_lid = @biddingId ORDER BY pr.id DESC  OFFSET (@pageNo - 1) * ${showEntry} ROWS FETCH NEXT ${showEntry} ROWS ONLY`)
            })
        }
        else{
            return poolConnection.then(pool=>{
                return pool.request() 
                .input('biddingId', sql.Int, biddingId)
                .query(`SELECT TOP ${showEntry} pr.id, p.program_name, pr.acad_session, pr.id,
