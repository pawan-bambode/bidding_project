...
module.exports = class StudentsData {
    ...
    static getCounts(slug,biddingId){
        return poolConnection.then(pool=>{
          return pool.request() 
          .input('biddingId',sql.Int,biddingId)
          .query(` SELECT COUNT(*) FROM [${slug}].student_data sd INNER JOIN [${slug}].programs p ON p.program_id = sd.program_id WHERE sd.active = 1  AND  p.bidding_session_lid= @biddingId  AND sd.bidding_session_lid = @biddingId`)
        })
    }
    static searchStudentData(slug,biddingId,letterSearch,userId,pageNo,showEntry){
        ...
        .input('last_modified_by',sql.Int,userId)
        .input('biddingId',sql.Int,biddingId)
        .query(`SELECT   sd.id ,sd.sap_id, sd.roll_no,sd.student_name,sd.email,p.program_name,sd.bid_points,sd.year_of_joining ,sd.previous_elective_credits
        FROM [${slug}].student_data sd 
...
static searchStudentDataByletter(slug, biddingId, letterSearch,pageNo,showEntry) {
    console.log('inside the searchStudentDataByletter');
    if(pageNo){
        console.log('inside the if block',pageNo);
        if(letterSearch){
            console.log('inside the if block',letterSearch);
    return poolConnection.then(pool => {
        return pool.request()
            .input('pageNo',sql.Int,pageNo)
            .input('bidding_session_lid', sql.Int, biddingId)
...
static studentDataFilterByProgramId(slug,biddingId,programId,showEntry){
    ...
    return poolConnection.then(pool =>{
        ...
        .query(`SELECT TOP ${showEntry} sd.sap_id,sd.roll_no,sd.student_name,sd.email,p.program_name,sd.bid_points,sd.year_of_joining
        FROM [${slug}].student_data sd
        INNER JOIN [${slug}].programs p ON p.program_id = sd.program_id 
...
return poolConnection.then(pool =>{
    ...
    .input('programId',sql.Int,programId)
    .input('sapId',sql.NVarChar(sql.MAX),sapId)
    .query(`SELECT COUNT(*) FROM [${slug}].student_data sd INNER JOIN [${slug}].programs p ON p.program_id = sd.program_id 
    WHERE sd.active = 1 AND sd.bidding_session_lid = @biddingId AND p.program_id = @programId AND p.bidding_session_lid = @biddingId AND sd.sap_id = @sapId`)
  })
}}